...
loglm
```
#cece: remember to remove NAs, you can use "na.action = na.exclude". Otherwise looks good
...
```
#Cece: This is the method I used as well, looks good!
...
#Cece: Looks good, very similar to what I got!
...
```{r 95% CI}
c <-boot.ci(boot.out = bootobject, conf = 0.95, type = 'norm', index = 1)
c
d <- boot.ci(boot.out = bootobject, conf = 0.95, type = 'norm', index = 2)
d
```

#Cece: I struggled on this part as I was only able to get B0 and not B1 so this is really helpful! I also chose "norm". Just remember to name each function and then call the name of that function, not boot.ci. When you call boot.ci it just brings up the code associated with that funcion. I renamed them above so I could see their output. 
...
```
#Cece: I have no idea how to do the extra credit so kudos to you for giving it a shot. Also make sure to add the challeges you faced at the end of the document for the final push. 
# **Extra Extra Credit**
